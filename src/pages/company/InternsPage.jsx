...
const CompanyInternsPage = () => {
  ...
  const activeInterns = STUDENTS_LIST.filter(
    (s) =>
      s.company === 'Zoho Corp' ||
  ...
  return (
    <div className="flex flex-col w-full space-y-6">
      <PageHeader
        ...
        badge={<span className="text-xs font-semibold px-2.5 py-0.5 bg-primary-fixed/40 text-primary rounded-full">{activeInterns.length} Active</span>}
        actions={[
          { label: '+ Assign Task', onClick: () => navigate(ROUTES.COMPANY.TASKS), variant: 'primary' },
        ]}
      />

      <div className="bg-surface-container-lowest rounded-xl shadow-sm p-6 space-y-6">
        <SearchBar placeholder="Search active interns..." value={searchTerm} onChange={setSearchTerm} />
            ...
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h4 className="font-title-md text-title-md font-bold text-on-surface">{intern.name}</h4>
                    <p className="font-body-sm text-body-sm text-on-surface-variant">{intern.role}</p>
                  </div>
                </div>
                <ProgressRing value={intern.progress} size={48} label={`${intern.progress}%`} />
              </div>
              ...
              <div className="pt-2 border-t border-surface-container-high grid grid-cols-2 gap-2 font-body-sm text-body-sm">
                <div>
                  ...
                  <span className="font-semibold text-emerald-600">{intern.attendance}%</span>
                </div>
                <div>
                  <span className="text-on-surface-variant block">Last Report:</span>
                  <span className="font-semibold text-on-surface">{intern.lastReport || 'N/A'}</span>
                </div>
              </div>
